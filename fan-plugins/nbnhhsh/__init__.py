...
__plugin_meta__ = PluginMetadata(
    ...
    description="能不能好好说话 - 使用神奇海螺api查询",
    usage="xxxx是什么",
    config=Config,
)
...
regular_expression = r"[a-z0-9]{2,}+(?:是什么|是啥|啥)"
...
async def handle_nbnhhsh(keyword: str = RegexStr()):
    ...
    keyword_string = keyword.replace("是什么", "").replace("是啥", "").replace("啥", "")
...
async def query_nbnhhsh(keyword: str) -> dict:
    ...
    try:
        # 如果有正在进行的请求，终止它
        if hasattr(query_nbnhhsh, "_request"):
            query_nbnhhsh._request.close()
            query_nbnhhsh._request = None

        # 发送POST请求
        query_nbnhhsh._request = requests.post(
        ...
        # 解析响应
        data = query_nbnhhsh._request.json()
